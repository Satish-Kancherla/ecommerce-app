...
type UpdateProductModalProps = {
  ...
  isOpen: boolean;
  onClose: () => void;
  onUpdate: (updatedProduct: WomenItem) => void;
};

const UpdateProductModal = ({ product, isOpen, onClose, onUpdate }: UpdateProductModalProps) => {
  const [updatedProduct, setUpdatedProduct] = useState<WomenItem>(product);
  ...
  const handleSubmit = async () => {
    try {
      const res = await axios.put(`/api/women/update?id=${updatedProduct.id}`, updatedProduct);
      if (res.status === 200) {
